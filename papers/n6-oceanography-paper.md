<!-- @canonical-origin: canon@a86ca143:papers/n6-oceanography-paper.md (moved 2026-05-10) -->
<!-- gold-standard: shared/harness/sample.md -->
---
domain: oceanography
requires: []
---
# [CANONICAL v2] Ultimate oceanography (HEXA-OCEANOGRAPHY) — n=6 Arithmetic Coordinate Mapping

> **Author**: Minwoo Park (canon)
> **Category**: oceanography — n=6 arithmetic seed paper
> **Version**: v2 (2026-04-14 canonical)
> **Upstream BT**: BT-375, BT-375, BT-375, BT-375, BT-375
> **Linked atlas node**: `oceanography` 28/28 EXACT [10*]

---

## 0. Abstract

This paper demonstrates that the core parameters of the oceanography domain
...
oceanography. atlas.n6 records 28/28 entries EXACT.

This paper does not claim a new oceanography; it is a seed paper that
...
oceanography is re-read within the n=6 arithmetic framework. The perfect
...
of the oceanography domain. **This paper overlays an n=6 arithmetic
coordinate system on top of existing oceanography knowledge.**

| Effect | Before | After HEXA-OCEANOGRAPHY | Perceived change |
|--------|--------|--------------|------------------|
...
```
  Before: "Why is this oceanography value this number?" → experience/custom
  HEXA:   "This oceanography value = σ(6) or τ(6) or sopfr(6)" → number-theoretic necessity
         ↓
...
## §2 COMPARE (classical oceanography vs n=6) — performance comparison (ASCII)
...
### Performance-bar ASCII (classical oceanography vs HEXA-OCEANOGRAPHY)
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                    HEXA-OCEANOGRAPHY            system structure          │
├────────────┬────────────┬────────────┬────────────┬─────────────────────┤
...
4. **Domain overlap**: the σ=12 axes are shared parameters across dozens of domains beyond oceanography.
...
HEXA-OCEANOGRAPHY staged maturity roadmap — verification density grows per Mk:

<details open>
<summary><b>Mk.V — 2045+ full integration</b></summary>

All of oceanography is integrated with n=6 arithmetic. Cross-referenced
...
oceanography core parameters are mapped onto σ/τ/φ/sopfr/J₂.
...
Check whether the main oceanography scaling laws follow the τ(6)=4 or
...
Compute atlas 28/28 EXACT under H₀ (random matching) → p-value.
...
#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# §7 VERIFY -- HEXA-OCEANOGRAPHY n=6 honesty check (stdlib only, oceanography domain)
#
...
FALSIFIERS = [
    "If oceanography key parameter n=6 alignment < 70%, retract the paper's main target",
    "If sigma(n)*phi(n) = n*tau(n) holds for any n other than n=6, retract the uniqueness candidate",
    "If atlas 28/28 EXACT drops below 70% on re-measurement, demote to Mk.I",
